...
const seedDB = async () => {
    try {
        ...
        if (!user) {
            console.error("No recruiter user found in DB. Please register one first.");
            process.exit(1);
        }

        console.log("Using recruiter user:", user.fullname, `(${user._id})`);

        const newJobs = generateJobs(user._id);
